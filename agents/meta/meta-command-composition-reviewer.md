...
3. **Long-running commands invoke `meta-session-planner` as their first step.** A "long-running" command is one whose `phase` is not `meta` or `lifecycle` AND whose body lists more than three discrete steps. Commands that fit that definition but whose body does not invoke the planner are a violation — **unless** the command declares `fast-command: true` in its frontmatter. That marker is the sanctioned fast-command exemption from `commands/CLAUDE.md` rule 3: a fixed-cost command opts out of the planner. Honour it — do not flag a `fast-command: true` command for a missing planner.
...
2. **Parse each command.** Extract the YAML frontmatter (expect the keys documented in `commands/CLAUDE.md`: `context`, `model`, `allowed-tools`, `argument-hint`, `description`, and the optional `fast-command` rule-3 exemption marker). Extract the first H2 or the first non-frontmatter paragraph as the opening statement.
...
5. **Session-planner invocation check (rule 3).** For each command whose `phase` is not `meta`/`lifecycle`, first check for `fast-command: true` in its frontmatter — if present, the command is exempt; skip it (record it under `notes` as a sanctioned fast-command). Otherwise count the numbered protocol steps in the body. If the count exceeds 3, search the body for a reference to `meta-session-planner` (or an equivalent planning step that instantiates it). No match is `missing-session-planner` — blocking.

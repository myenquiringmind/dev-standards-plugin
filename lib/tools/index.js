...
function formatFile(filePath) {
  ...
  const pathValidation = validation.validateFilePath(filePath);
  if (!pathValidation.valid) {
    logging.warn(`Invalid file path for format: ${pathValidation.reason}`);
    return { success: false, error: pathValidation.reason };
  }
...
function typeCheckFile(filePath) {
  ...
  const pathValidation = validation.validateFilePath(filePath);
  if (!pathValidation.valid) {
    logging.warn(`Invalid file path for typecheck: ${pathValidation.reason}`);
    return { success: false, error: pathValidation.reason };
  }
...
function lintFile(filePath) {
  ...
  const pathValidation = validation.validateFilePath(filePath);
  if (!pathValidation.valid) {
    logging.warn(`Invalid file path for lint: ${pathValidation.reason}`);
    return { success: false, warning: pathValidation.reason };
  }
